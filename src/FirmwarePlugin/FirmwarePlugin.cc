...
void FirmwarePlugin::guidedModeGotoLocation(Vehicle* vehicle, const QGeoCoordinate& gotoCoord, double altitude)
{
    // Not supported by generic vehicle
    Q_UNUSED(vehicle);
    Q_UNUSED(gotoCoord);
...
const QVariantList &FirmwarePlugin::toolBarIndicators(const Vehicle*)
{
    ...
    if(_toolBarIndicatorList.size() == 0) {
        _toolBarIndicatorList = QVariantList({
            ...
            QVariant::fromValue(QUrl::fromUserInput("qrc:/toolbar/ReadyIndicator.qml")),
